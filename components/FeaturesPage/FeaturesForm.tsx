...
export default function FeedbackForm() {
    ...
    const { startUpload, isUploading, permittedFileInfo } = useUploadThing('imageUploader', {
        onClientUploadComplete: (res) => {
            toast({
                ...
                description: `${(res || []).length} images was loaded successfully`,
                variant: 'destructive',
            })
            setFiles([...files, ...res])
    ...
    const { mutate: submitForm, isLoading } = useMutation({
        ...
        onSuccess: () => {
            toast({
                title: 'Success',
                description: 'Your feature was submited successfuly and ll be reviewd by our team. If you submit your contacts we ll reach you',
                variant: 'destructive',
            })
        },
    })
...
function SubmitButton() {
    return <div className="flex justify-end pt-4">
        <button
            className="group relative flex left select-none items-center justify-center gap-2 px-3 py-1 outline-none focus-visible:ring-4 bg-purple-600 hover:bg-purple-700 rounded h-[36px] whitespace-nowrap text-xs transition ml-2 cursor-pointer"
            type="submit"
        >
