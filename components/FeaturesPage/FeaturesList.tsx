        ...
        <div className="flex w-full flex-col px-4">
            ...
            {!isFetching ? <>
                <div className="flex items-center px-2 mt-6">
                    ...
                    <div className="ml-2 text-lg font-semibold text-purple-500">In Progress</div>
                </div>
                <div className="flex-1">
                    {data.data.filter(r => r.status === "In Process").map((r, idx) => {
                ...
                <div className="flex items-center px-2 border-t pt-4 border-zinc-600">
                    ...
                    <div className="ml-2 text-lg font-semibold text-purple-500">Planed</div>
                </div>
                <div className="flex-1">
                    {data.data.filter(r => r.status === "Planned").map((r, idx) => {
                        return (<Row key={r.id} idx={idx} page={r} />
                        )
                    })}
                </div>
                <div className="flex items-center px-2  border-t border-zinc-600 pt-4">
                    ...
                    <div className="ml-2 text-lg font-semibold text-purple-500">Backlog</div>
                </div>
                <div className="flex-1">
                    {data.data.filter(r => r.status === "Backlog").map((r, idx) => {
                        return (<Row key={r.id} idx={idx} page={r} />
                        )
                    })} </div>
